<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    @auth
        <span class="text-lg font-semibold text-gray-800 dark:text-gray-200">
            {{ Auth::user()->role === 'seller' ? 'Seller Dashboard' : 'Buyer Dashboard' }}
        </span>
    @endauth
</div>
        ...
        <div class="pt-2 pb-3 space-y-1">
            @auth
                <x-responsive-nav-link
                    :href="Auth::user()->role === 'seller' ? route('dashboard.seller') : route('dashboard.buyer')"
                    :active="request()->routeIs('dashboard.seller') || request()->routeIs('dashboard.buyer')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
            @else
