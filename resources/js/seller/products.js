document.addEventListener('DOMContentLoaded', () => {
  ...
  function showModal(title) {
    ...
    document
      .getElementById('update-buttons')
      .classList.toggle('hidden', !isUpdate);
    document
  ...
  document.querySelectorAll('.action-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      form.description.value = row.querySelector(
        '.product-description'
      ...
      .then((result) => {
        ...
        if (isUpdate) {
          ...
          if (row) {
            ...
            row.querySelector('.product-name').dataset.fullName =
              form.name.value;
            const desc = form.description.value;
            row.querySelector('.product-description').textContent =
              desc.length > 60 ? desc.slice(0, 60) + '…' : desc;
            row.querySelector('.product-description').dataset.fullDescription =
              desc;
            row.querySelector('.product-price').textContent = parseFloat(
              form.price.value
            ).toFixed(2);
